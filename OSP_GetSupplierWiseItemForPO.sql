...
(  
	...
	(  
		...
		(
			SELECT DISTINCT MCODE, DIVISION, IIF(@BCStock=1,BC,'') BC,  SUM(REALQTY_IN - REALQTY) OVER (PARTITION BY MCODE) STOCK, SUM(REALQTY_IN - REALQTY) OVER (PARTITION BY DIVISION,MCODE,IIF(@BCStock=1,BC,'')) outletStock FROM RMD_TRNPROD WITH (NOLOCK)
		) A ON MI.MCODE = A.MCODE WHERE ISNULL(A.DIVISION, @Division) LIKE @Division AND MI.TYPE = 'A'
