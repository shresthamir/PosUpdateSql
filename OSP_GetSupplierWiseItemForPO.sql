...
(  
	...
	(  
		...
		(
			...
			, SUM(REALQTY_IN - REALQTY) OVER (PARTITION BY IIF(@DIVISION = '%', '%', DIVISION),MCODE,IIF(@BCStock=1,BC,'')) outletStock FROM RMD_TRNPROD A WITH (NOLOCK) WHERE PhiscalID = @PhiscalID
