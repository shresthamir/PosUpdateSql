CREATE OR ALTER PROC[dbo].[OSP_CompileCOGS]    
--DECLARE     
@DIVISION VARCHAR(3) ,    
...
BEGIN TRY    
 ...
 SELECT P.MCODE, SUM(REALQTY - REALQTY_IN) SOLD_QTY INTO #SALES_DATA  FROM RMD_TRNPROD P JOIN RMD_TRNMAIN M ON P.VCHRNO = M.VCHRNO WHERE P.VOUCHERTYPE IN ('SI','TI','CN') AND P.DIVISION Like @DIVISION AND P.PhiscalID = @PhiscalId  
 AND M.TRNDATE BETWEEN @DATE1 AND @DATE2  
 GROUP BY P.MCODE    
 ...
 END    
  ...
  create table #VOUCHERLIST ( VCHRNO VARCHAR(25), VoucherType VARCHAR(2), TRNDATE DATETIME, MCODE VARCHAR(30) ,QTY NUMERIC(18,2))    
  
  IF OBJECT_ID('TempDB..#DISTINCTVOUCHERLIST') IS  NOT NULL DROP TABLE #DISTINCTVOUCHERLIST    
  CREATE TABLE #DISTINCTVOUCHERLIST ( VCHRNO VARCHAR(25), VoucherType VARCHAR(2), TRNDATE DATETIME)    
  ...
  select TM.VCHRNO, TM.VoucherType, TM.TRNDATE + TM.TRNTIME, TP.MCODE , SUM(TP.Quantity) Qty from SALES_TRNMAIN TM    
  ...
  GROUP BY TM.VCHRNO, TM.VoucherType, TM.TRNDATE, TM.TRNTIME, TP.MCODE, TM.stamp     
  order by TM.stamp     
    
  INSERT INTO #DISTINCTVOUCHERLIST    
  SELECT DISTINCT VCHRNO, VOUCHERTYPE, TRNDATE FROM #VOUCHERLIST    
    
  DECLARE @i INTEGER, @COUNT INTEGER;    
  DECLARE @VoucherNo Varchar(25), @VoucherType VARCHAR(2)   
  ...
  BEGIN    
   ...
   DELETE FROM #CMI  
   Select @VoucherNo = VCHRNO, @VoucherType = VoucherType, @DATE = TRNDATE from #DISTINCTVOUCHERLIST order by CONVERT(INT, SUBSTRING(VCHRNO, 3, CHARINDEX('-',VCHRNO,3)-3)) asc, VCHRNO ASC offset @i rows fetch next 1 rows only  
   ...
   IF OBJECT_ID('TEMPDB..#Prod_COGS') IS NOT NULL DROP TABLE #Prod_COGS
   ...
   COALESCE(CP.RATE, CMI.VAL_RATE, NULLIF(MI.CRATE,0), MI.PRATE_A) CRATE, Convert(Float,GETDATE()) STAMP INTO #Prod_COGS from RMD_TRNPROD RP   
   ...
   where RP.VCHRNO = @VoucherNo  
	...
	IF @VoucherType IN ('TI','CN')
		INSERT INTO TRNPROD_COGS(VCHRNO, VOUCHERTYPE, MCODE, BC, BATCH, SNO, COSTING_INVOICE, Quantity, CRATE, STAMP)  
		SELECT * FROM #Prod_COGS
	ELSE 
		INSERT INTO ABBPROD_COGS(VCHRNO, VOUCHERTYPE, MCODE, BC, BATCH, SNO, COSTING_INVOICE, Quantity, CRATE, STAMP)  
		SELECT * FROM #Prod_COGS
