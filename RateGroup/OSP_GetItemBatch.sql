...
SELECT B.BATCHCREATEDATE,P.MCODE, IIF(@bcStock = 1 AND MI.HASVARIANT = 1, P.BC, @BARCODE) BCODE, B.BATCHCODE, B.MFGDATE, B.EXPDATE, COALESCE(C.RG_RATE, B.SELLRATEBEFORETAX) SELLRATEBEFORETAX, ISNULL(C.RG_MRP, B.MRP) MRP, LABELMRP, SUM(REALQTY_IN-REALQTY) STOCK,       
...
GROUP BY B.BATCHCREATEDATE, P.MCODE, IIF(@bcStock = 1 AND MI.HASVARIANT = 1, P.BC, @BARCODE), B.BATCHCODE, B.MFGDATE, B.EXPDATE, B.MRP,B.LABELMRP, B.SELLRATEBEFORETAX, ISNULL(B.CostPrice,P.PRATE), B.W_MRP, B.W_SELLRATEBEFORETAX, C.RG_RATE, C.RG_MRP, B.LandingCost, D.RG_RATE, D.RG_MRP, C.RG_DISCOUNT, D.RG_DISCOUNT      
HAVING @OnlyInStock = 0 OR SUM(REALQTY_IN - REALQTY) > 0      
ORDER BY B.EXPDATE,B.BATCHCREATEDATE