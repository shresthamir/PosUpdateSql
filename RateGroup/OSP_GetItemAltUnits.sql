...
SELECT A.MCODE, A.ALTUNIT, A.CONFACTOR, A.ISDEFAULT, A.ISDEFAULTPRATE, A.BRCODE, A.PRATE, A.CRATE,
...
(
	SELECT A.MCODE, A.BASEUNIT ALTUNIT, 1 CONFACTOR, 0 ISDEFAULT, 0 ISDEFAULTPRATE, NULL BRCODE, ISNULL(NULLIF(A.PRATE_A,0), A.CRATE) PRATE, ISNULL(NULLIF(A.CRATE,0), A.PRATE_A) CRATE, A.RATE_A RATE, RATE_B, 0 SRATE_DISCOUNT, 0 WSRATE_DISCOUNT FROM MENUITEM A
	WHERE A.MCODE = @MCODE
	UNION ALL 
	SELECT A.MCODE, A.ALTUNIT, A.CONFACTOR, A.ISDEFAULT, A.ISDEFAULTPRATE, A.BRCODE, 
	CASE WHEN A.PRATE = 0 THEN ISNULL(NULLIF(M.PRATE_A,0), M.CRATE) * A.CONFACTOR ELSE A.PRATE END PRATE, 
	ISNULL(NULLIF(M.CRATE,0), M.PRATE_A) * A.CONFACTOR CRATE,	
