...
CAST(ISNULL(E.AMOUNT, 0) AS decimal(20,2)) AS COUNTERAMOUNT, FB.FoodSales, FB.BeverageSales, FB.OtherSales, D.BILLS, D.[RETURN_BILLS]
...
( 
	...
	SUM(BILLS) BILLS, SUM([RETURNS]) RETURN_BILLS,
	SUM(IIF(VoucherType = 'CN',0, NETAMNT)) TOTALSALES,
	SUM(IIF(VoucherType != 'CN',0, NETAMNT)) TOTALRETURN FROM 
	(
		SELECT VoucherType, SHIFT,TOTAMNT,DCAMNT,VATAMNT,NETAMNT,TAXABLE,NONTAXABLE,DIVISION,PhiscalID, 1 BILLS, 0 [RETURNS] FROM ABBMAIN 
		UNION ALL 
		...
		, CASE WHEN VoucherType = 'CN' THEN 1 ELSE 0 END  FROM TRNMAIN 
	)
