CREATE OR ALTER   PROC [DBO].[RSP_PRODUCTION_CONSUMPTION_REPORT]
	...
	SELECT IM.VCHRNO,IM.TRNDATE,IM.BSDATE,IM.REFBILL,IVP.MCODE PRODUCTION_ITEM,IVP.RATE PDRATE,IVP.REALQTY_IN,IVP.UNIT ,IVP.WAREHOUSE PD_WAREHOUSE ,IVC.MCODE CONSUMPTION_ITEM,IVC.WAREHOUSE PC_WAREHOUSE
	 ,IVC.RATE CONS_RATE,IVC.RealQty,
	IVC.UNIT CONS_UNIT ,IVC.REMARKS,ROW_NUMBER() OVER(PARTITION BY IM.VCHRNO,IVP.MCODE ORDER BY IM.VCHRNO,IM.TRNDATE) XNO INTO #DATA FROM INVMAIN IM JOIN INVPROD IVP ON  IM.VCHRNO=IVP.VCHRNO  AND IVP.REALQTY_IN>0
	...
	IIF(XNO=1,A.REFBILL,'') REFNO,IIF(XNO=1,DESCA,'')  DESCA, IIF(XNO=1,A.PD_WAREHOUSE,'')  PD_WAREHOUSE,  IIF(XNO=1,CONVERT(VARCHAR(100),A.PDRATE),'')  PDRATE ,IIF(XNO=1,CONVERT(VARCHAR(100),A.REALQTY_IN),'') PDQTY,IIF(XNO=1,A.UNIT ,'')PDUNIT,A.CONSUMPTION_ITEM,A.PC_WAREHOUSE,A.CONS_RATE ,A.RealQty CON_QTY ,A.CONS_UNIT,A.REMARKS   FROM
	    
	 (
	 SELECT  RWN.ROWN SN ,A.VCHRNO  ,  A.TRNDATE,A.BSDATE,
	  A.REFBILL ,PDM.DESCA, A.PD_WAREHOUSE, A.PDRATE ,A.REALQTY_IN  ,A.UNIT  ,PDC.DESCA CONSUMPTION_ITEM,A.PC_WAREHOUSE,A.CONS_RATE ,A.RealQty   ,A.CONS_UNIT,A.REMARKS,'A' FLG ,A.XNO from #DATA A JOIN MENUITEM PDM ON PDM.MCODE=A.PRODUCTION_ITEM
	 ...
	 UNION
	   ...
	   SELECT DISTINCT   RWN.ROWN   , A.VCHRNO ENTRY_NO , TRNDATE  DATE,NULL BSDATE,
