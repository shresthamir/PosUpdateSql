...
ELSE
	...
	WHERE AV.VCHRNO IS NULL AND X.MGROUP LIKE @MGROUP AND B.TRNDATE<=@DATE AND B.PhiscalID LIKE @FYID AND B.DIVISION LIKE @DIVISION
	...
	WHERE A.BATCH LIKE @BATCH AND A.WAREHOUSE LIKE @WARE AND X.MGROUP LIKE @MGROUP  AND A.PhiscalID LIKE @FYID
...
ELSE IF @FLAG = 7	-- PHYSICAL STOCK ONLY	
	...
	AND A.BATCH LIKE @BATCH AND (A.TRNDATE >= @DATE AND A.TRNDATE <= @DATE) AND WAREHOUSE LIKE @WARE AND C.MGROUP LIKE @MGROUP  AND A.PhiscalID LIKE @FYID AND C.PTYPE < 10
