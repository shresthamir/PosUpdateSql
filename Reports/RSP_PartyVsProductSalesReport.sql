CREATE OR ALTER PROC RSP_PartyVsProductSalesReport
--DECLARE
@DATE1 DATE,
@DATE2 DATE,
@DIVISION VARCHAR(3) = '%',
...
AS
--SET @DATE1 = '2020-07-16'; SET @DATE2 = '2021-07-15';SET @DIVISION = '%'
...
IIF(M.VoucherType IN ('SI','TI'), 1, -1) * CONVERT(NUMERIC(15,2), P.NETAMOUNT) NETAMOUNT INTO #DATA
...
IF @OPT_SHOWSUMMARY_REPORT = 1
	...
	(
		...
		SUM(NETSALE) NETSALE, SUM(TAXABLE) TAXABLE, SUM(NONTAXABLE) NONTAXABLE, SUM(D.VAT) VAT, SUM(NETAMOUNT) NETAMOUNT FROM #DATA D
		...
		GROUP BY ACID, ACNAME, AREA_NAME, CATEGORY_NAME, BC, BATCH, m.MCODE, M.MENUCODE, M.DESCA
...
ELSE 
	...
	D.SERVICETAX, D.NETSALE, D.TAXABLE, D.NONTAXABLE, D.VAT, D.NETAMOUNT, BD.* FROM #DATA D
	JOIN dbo.fnProductFilter(@SELECTEDITEMLIST, @MENUCAT, default, @MGROUP, @PTYPE, @SUPPLIER_ACID,default, default, default, default, default, default) M ON D.MCODE = M.MCODE
