...
(
	SELECT A.MCODE,A.OPQTY,A.PQTY,A.TRANSFERIN,A.SALESQTY,A.TRANSFEROUT,A.STOCKADJUSTMENT,A.SETTLEMENT,A.PRODUCTION,A.STOCKBALANCE,
	...
	(
		...
		PRODUCTION=SUM(CASE WHEN LEFT(VCHRNO,2) IN ('PD') THEN  REALQTY_IN-REALQTY ELSE 0 END ),
...
SETTLEMENT NUMERIC(18,4),PRODUCTION NUMERIC(18,4),STOCKBALANCE NUMERIC(18,4),PRATE NUMERIC(18,4),STOCKVALUE NUMERIC(18,4),SRATE NUMERIC(18,4),SUPPLIER VARCHAR(max) COLLATE DATABASE_DEFAULT,MCODE VARCHAR(max) COLLATE DATABASE_DEFAULT)
...
(
	...
	OPQTY,PQTY,TRANSFERIN,SALESQTY,TRANSFEROUT,STOCKADJUSTMENT,SETTLEMENT,PRODUCTION,STOCKBALANCE,STOCKVALUE
...
(ITEMCODE,DESCA,BARCODE,BASEUNIT,OPQTY,PQTY,TRANSFERIN,SALESQTY,TRANSFEROUT,STOCKADJUSTMENT,SETTLEMENT,PRODUCTION,STOCKBALANCE,PRATE,STOCKVALUE,SRATE,SUPPLIER,VAT,MCODE)
SELECT * FROM
(
	...
	SUM(STOCKADJUSTMENT) STOCKADJUSTMENT,SUM(SETTLEMENT) SETTLEMENT,SUM(PRODUCTION) PRODUCTION,SUM(STOCKBALANCE) STOCKBALANCE,
...
BASEUNIT VARCHAR(max),OPQTY NUMERIC(18,4),PQTY NUMERIC(18,4),TRANSFERIN NUMERIC(18,4),SALESQTY NUMERIC(18,4),TRANSFEROUT NUMERIC(18,4),STOCKADJUSTMENT NUMERIC(18,4),
SETTLEMENT NUMERIC(18,4),PRODUCTION NUMERIC(18,4),STOCKBALANCE NUMERIC(18,4),PRATE NUMERIC(18,4),STOCKVALUE NUMERIC(18,4),SRATE NUMERIC(18,4),SUPPLIER VARCHAR(max),VAT VARCHAR(15),MCODE VARCHAR(max),
...
if @SHOWBARCODEDETAIL = 0
	BEGIN
		IF @BYBARCODE = 0 OR @WISE <> 'ITEM'
			...
			SUM(TRANSFEROUT)TRANSFEROUT,SUM(STOCKADJUSTMENT) STOCKADJUSTMENT,SUM(SETTLEMENT) SETTLEMENT,SUM(PRODUCTION) PRODUCTION,
	...
	BEGIN
		...
		if @SHOWBARCODEDETAIL = 0
			BEGIN
				...
				B.BASEUNIT,OPQTY,PQTY,TRANSFERIN,SALESQTY,TRANSFEROUT,STOCKADJUSTMENT,SETTLEMENT,PRODUCTION,STOCKBALANCE,PRATE,STOCKVALUE,SUPPLIER,
				...
				BEGIN
					...
					A.STOCKADJUSTMENT =B.STOCKADJUSTMENT ,A.SETTLEMENT =B.SETTLEMENT , A.PRODUCTION =B.PRODUCTION  ,A.PRATE =B.PRATE  ,A.STOCKVALUE =B.STOCKVALUE,A.STOCKBALANCE=B.STOCKBALANCE 
					...
					SUM(STOCKADJUSTMENT ) STOCKADJUSTMENT,SUM(SETTLEMENT ) SETTLEMENT,SUM(PRODUCTION) PRODUCTION,SUM(STOCKBALANCE ) STOCKBALANCE,AVG(PRATE ) PRATE,SUM(STOCKVALUE ) STOCKVALUE from #TREE where level= @LVL group by parent) B on A.MCODE =B.Parent 
					SET @lvl = @lvl - 1;
				END

				SELECT * FROM #TREE where OPQTY <> 0 or PQTY <>0 or TRANSFERIN=0 or TRANSFEROUT=0 or SALESQTY <> 0 or TRANSFEROUT <> 0 or STOCKADJUSTMENT <> 0 or SETTLEMENT <> 0 or PRODUCTION <> 0 ORDER BY ID
			END
		ELSE
			BEGIN
				...
				BEGIN
					...
					(select Parent,SUM(OPQTY) OPQTY,SUM(PQTY  ) PQTY,SUM(TRANSFERIN ) TRANSFERIN,SUM(SALESQTY ) SALESQTY,SUM(TRANSFEROUT ) TRANSFEROUT,
					SUM(STOCKADJUSTMENT ) STOCKADJUSTMENT,SUM(SETTLEMENT ) SETTLEMENT,SUM(PRODUCTION) PRODUCTION,SUM(STOCKBALANCE ) STOCKBALANCE,AVG(PRATE ) PRATE,SUM(STOCKVALUE ) STOCKVALUE from #TREE1 where level= @LVL group by parent) B on A.MCODE =B.Parent 
					SET @lvl = @lvl - 1;
				END

				SELECT * FROM #TREE1 where OPQTY <> 0 or PQTY <>0 or TRANSFERIN=0 or TRANSFEROUT=0 or SALESQTY <> 0 or TRANSFEROUT <> 0 or STOCKADJUSTMENT <> 0 or SETTLEMENT <> 0 or PRODUCTION <> 0 ORDER BY ID
			END
