...
MI.[RATE_A] * MRP_FACTOR MRP, MI.[RATE_B], MI.[RATE_B] * MI.MRP_FACTOR W_MRP,  PRATE_A, MI.PRATE_A * MI.MRP_FACTOR [PRATE_INC_VAT], CRATE
...
IF @OPT_TREE = 0 
BEGIN
	SELECT A.*, B.ACCODE SuplierCode, B.ACNAME Supplier FROM #RESULT A LEFT JOIN RMD_ACLIST B ON A.SUPCODE = B.ACID 
END
ELSE 
BEGIN
	...
	SELECT A.ID, A.TYPE, A.DESCRIPTION, B.*, C.ACCODE SuplierCode, C.ACNAME Supplier INTO #TREE FROM TreeExpand_function (@Group ,@GroupName ,0) AS A
