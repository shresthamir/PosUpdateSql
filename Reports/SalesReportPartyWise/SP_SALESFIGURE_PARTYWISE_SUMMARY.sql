...
IF @PTYPE= 0 	-- CUSTOMER WISE
	...
	(
	SELECT CNAME ACNAME,BILLTOTEL VATNO,B.PARTYTYPE,CONVERT(DECIMAL(18,2), SUM(TOTAMNT)) AS GROSS, 
	...
	CONVERT(DECIMAL(18,2), SUM(VATAMNT)) AS VATAMNT, CONVERT(DECIMAL(18,2), SUM(NETAMNT)) AS BILLAMNT ,ca.AREA_NAME,ca.CATEGORY_NAME
	FROM TRNMAIN_VIEW_SREPORT A 
	LEFT JOIN #PARTYTYPE B ON A.RATETYPE = B.ID
	LEFT join #PARTYFILTER ca on ca.ACID=a.TRNAC
	WHERE DIVISION LIKE @DIV AND TRNDATE BETWEEN @DATE1 AND @DATE2 AND (TRNAC LIKE @ACID OR RECEIVEBY LIKE @ACID) AND
...
ELSE
	...
	FROM 
	(
	SELECT NULL AS SNO,ISNULL(B.ACNAME,'UNKNOWN SUPPLIER') AS ACNAME,B.VATNO,NULL PTYPE, CONVERT(DECIMAL(18,2), SUM(AMOUNT)) AS GROSS, 
	CONVERT(DECIMAL(18,2), (SUM(TOTALLOYALTY)/SUM(TOTAMNT))*100) [LOYALTY DISCOUNT(%)],CONVERT(DECIMAL(18,2), SUM(DISCOUNT)) AS DISCOUNT,
	...
	CONVERT(DECIMAL(18,2), SUM(VATAMNT)) AS VATAMNT,CONVERT(DECIMAL(18,2), SUM(NETAMOUNT)) AS NETAMOUNT,B.ACID,ca.AREA_NAME,ca.CATEGORY_NAME
	...
	join #PARTYFILTER ca on ca.ACID=c.TRNAC
	...
	GROUP BY B.ACNAME,B.ACID,B.VATNO,ca.AREA_NAME,ca.CATEGORY_NAME
	HAVING SUM(AMOUNT)<>0
