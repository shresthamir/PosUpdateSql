...
IF @PTYPE= 0 	-- CUSTOMER WISE
	SELECT ca.AREA_NAME,ca.CATEGORY_NAME, COALESCE(CP.CUSTNAME, CA.ACNAME, A.BillTo) ACNAME, COALESCE(CA.VATNO, CP.PANNO, A.BillToPan) VATNO, A.MENUCODE, A.DESCA, A.QUANTITY, A.RATE, A.AMOUNT, A.[LOYALTY DISCOUNT(%)], A.DISCOUNT
	, A.NETSALE, A.TAXABLE, A.NONTAXBLE, A.VAT, A.BILLAMNT FROM 
	(
		SELECT B.PARAC, B.RECEIVEBY, B.BillTo, B.BillToTel, MENUCODE, DESCA
		...
		GROUP BY MCODE,MENUCODE,DESCA,RATE, B.PARAC, B.RECEIVEBY, B.BillTo, B.BillToTel
		HAVING SUM(A.AMOUNT) <> 0
	) A LEFT JOIN #PARTYFILTER ca on ca.ACID = A.PARAC
	LEFT JOIN vwCustomerProfile cp on CP.CUSTID = A.RECEIVEBY
	WHERE (@AREA_ID = 0 OR CA.AREA_ID = @AREA_ID)
