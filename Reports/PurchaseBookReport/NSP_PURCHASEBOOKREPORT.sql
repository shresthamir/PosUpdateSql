...
--select @DATE1='2025-01-01',@DATE2='2025-02-12',@REPORT_TYPE=1,@SHOWDETAL_REPORT=0,@div='EXP',@INCLUDE_RETURN=1,@VOUCHER_TYPE = 'PI'
...
BEGIN	
	IF @REPORT_TYPE = 1		--VOUCHER WISE
		BEGIN
			...
			SELECT TRNDATE,DIVISION,BSDATE, ISNULL(BILLTO,'') AS BILLTO,VCHRNO,CHALANNO,TRNMODE,TRNAC,PARAC, TOTAMNT,DCAMNT,TAXABLE,NONTAXABLE,VATAMNT, NETAMNT,
			REMARKS,ISNULL(REFBILL,CHALANNO)REFBILL,ISNULL(VNUM,VCHRNO)VNUM,TRNTIME,TRNUSER,PHISCALID ,BILLTOTEL,TRN_DATE,BS_DATE
			...
			AND ISNULL(PARAC,TRNAC) LIKE @PARTY 

			SELECT A.TRNDATE,A.BSDATE, A.VCHRNO, A.REFBILL REFNO, A.TRN_DATE,A.BS_DATE,
			CASE WHEN A.TRNMODE = 'Cash' THEN CASE WHEN ISNULL(A.BILLTO,'') <> '' THEN A.BILLTO ELSE B.ACNAME END ELSE B.ACNAME END AS PARTY,
			CASE WHEN ISNULL(A.BILLTOTEL,'') = '' THEN B.VATNO ELSE A.BILLTOTEL END SUPPLIER_PAN, 
			A.TRNMODE,
			...
			#RMD_TRNMAIN AS A LEFT JOIN RMD_ACLIST B ON ISNULL(A.PARAC,A.TRNAC) = B.ACID LEFT JOIN DIVISION X ON A.DIVISION = X.INITIAL
			 
			
			IF @SHOWDETAL_REPORT = 0		-- SUMMARY REPORT					
				SELECT TRNDATE, BSDATE, VCHRNO, REFNO, TRNMODE, CONVERT(NUMERIC(18,2),TOTAMNT) TOTAMNT, CONVERT(NUMERIC(18,2),DCAMNT) DCAMNT, STAX, CONVERT(NUMERIC(18,2),NETSALE) NETSALE
				, CONVERT(NUMERIC(18,2),NONTAXABLE) NONTAXABLE, CONVERT(NUMERIC(18,2),TAXABLE) TAXABLE, CONVERT(NUMERIC(18,2),VATAMNT) VATAMNT, CONVERT(NUMERIC(18,2), TOTAL) TOTAL
				, REMARKS, PARTY,SUPPLIER_PAN, DIVNAME, TRNUSER, VNUM, DIVISION,TRN_DATE,BS_DATE, FLG FROM 
				(
					SELECT A.TRNDATE,A.BSDATE,A.VCHRNO,A.REFNO,A.TRNMODE, A.TOTAMNT,A.DCAMNT,0 STAX,A.NETSALE,A.NONTAXABLE,A.TAXABLE,A.VATAMNT, A.TOTAL, A.REMARKS,A.PARTY,SUPPLIER_PAN, A.DIVNAME,A.TRNUSER,A.VNUM,A.DIVISION,A.TRN_DATE,A.BS_DATE,'A' FLG FROM #TRNMAIN A
					UNION ALL
					SELECT NULL TRNDATE,NULL BSDATE,NULL VCHRNO,NULL REFNO,'TOTAL >>' TRNMODE,SUM(A.TOTAMNT),SUM(A.DCAMNT),0,SUM(A.NETSALE),SUM(A.NONTAXABLE),SUM(A.TAXABLE),SUM(A.VATAMNT),SUM(A.TOTAL),NULL REMARKS,NULL PARTY,NULL SUPPLIER_PAN,NULL DIVISION,NULL TRNUSER,'ZZ9999999' VNO,NULL DIVID,NULL TRN_DATE,NULL BS_DATE,'B' FLG FROM #TRNMAIN A
			...
			ELSE			-- DETAIL REPORT
				BEGIN
					...
					A.TAXABLE,A.NONTAXABLE,CASE WHEN (A.VAT = 0) THEN NULL ELSE A.VAT END AS VAT,(A.TAXABLE+A.NONTAXABLE+A.VAT) NETAMNT,B.VNUM,A.DIVISION,B.TRNDATE TDATE INTO #RMD_TRNPROD FROM vwPurprod A INNER JOIN
					#TRNMAIN B ON A.VCHRNO = B.VCHRNO AND A.DIVISION = B.DIVISION AND A.PHISCALID = B.PHISCALID
					INNER JOIN MENUITEM C ON A.MCODE = C.MCODE
			

					SELECT * FROM 
					(
					SELECT A.TRNDATE,A.BSDATE,A.VCHRNO,A.REFNO,A.PARTY,A.SUPPLIER_PAN,A.TRNMODE,X.QTY, NULL RATE,A.TOTAMNT,A.DCAMNT,0 STAX,A.NONTAXABLE,A.TAXABLE,A.VATAMNT,A.TOTAL,A.REMARKS,A.TRNTIME, A.TRNUSER,A.DIVNAME,A.VNUM,A.DIVISION,A.TRN_DATE,A.BS_DATE,'A' FLG,A.TRNDATE TDATE FROM #TRNMAIN A
					...
					UNION ALL
					SELECT A.TRNDATE,A.BSDATE,A.VCHRNO,A.MENUCODE,A.PRODUCT,NULL SUPPLIER_PAN,A.UNIT,A.QUANTITY,A.RATE,A.AMOUNT,A.DISCOUNT,0 SERVICETAX,A.NONTAXABLE,A.TAXABLE,A.VAT,A.NETAMNT,NULL PAXNO,NULL TRNTIME,NULL TRNUSER,NULL DIVISION,A.VNUM, NULL DIVID,NULL TRN_DATE,NULL BS_DATE,'B' FLG,A.TDATE FROM #RMD_TRNPROD A
					UNION ALL
					SELECT NULL TRNDATE,NULL BSDATE,NULL VCHRNO,NULL CHALANNO,NULL PARTY,NULL SUPPLIER_PAN,NULL TRNMODE,NULL QTY, NULL RATE,NULL TOTAMNT,NULL DCAMNT,NULL STAX,NULL NONTAXABLE,NULL TAXABLE,NULL VATAMNT,NULL TOTAL,NULL PAXNO,NULL TRNTIME, NULL TRNUSER,NULL DIVISION,A.VNUM,A.DIVISION DIVID,NULL TRN_DATE,NULL BS_DATE,'C' FLG,A.TRNDATE TDATE FROM #TRNMAIN A
					UNION ALL

					SELECT NULL TRNDATE,NULL BSDATE,NULL VCHRNO,NULL CHALANNO,'TOTAL >>' PARTY,NULL SUPPLIER_PAN,NULL TRNMODE,SUM(QTY) QTY, NULL RATE,SUM(A.TOTAMNT)TOTAMNT,SUM(A.DCAMNT)DCAMNT,
					...
					NULL DIVISION,'ZZ99999999'  VNUM,NULL DIVID,NULL TRN_DATE,NULL BS_DATE,'D' FLG,'01-01-2045' TDATE FROM 
					(SELECT A.TRNDATE,A.BSDATE,A.VCHRNO,A.REFNO,A.PARTY,A.TRNMODE,X.QTY, NULL RATE,A.TOTAMNT,A.DCAMNT,0 STAX,A.NONTAXABLE,A.TAXABLE,A.VATAMNT,A.TOTAL,NULL PAXNO,A.TRNTIME, A.TRNUSER,A.DIVNAME,A.VNUM,A.DIVISION,NULL TRN_DATE,NULL BS_DATE ,'A' FLG FROM #TRNMAIN A
					INNER JOIN (SELECT VNUM,DIVISION,SUM(CASE WHEN LEFT(VNUM,2) IN (@V4 ,@V5) THEN QUANTITY*-1 ELSE QUANTITY END) QTY FROM #RMD_TRNPROD GROUP BY VNUM,DIVISION) X ON A.VNUM =X.VNUM AND A.DIVISION = X.DIVISION
					) A
	...
	ELSE IF @REPORT_TYPE = 2		-- DAY WISE 		
		...
		LEFT(A.VCHRNO,2) IN (@V1,@V2,@V3,@V4,@V5) AND A.DIVISION LIKE @DIV AND (A.TRNDATE >=@DATE1 AND TRNDATE <= @DATE2) AND ISNULL(A.PARAC,A.TRNAC) LIKE @PARTY 
		) A
	...
	ELSE IF @REPORT_TYPE = 3				-- MONTH WISE
		...
		BEGIN
			...
			IF @INAD = 1	-- REPORT IN ENGLISH DATE				
				...
				CASE WHEN (TRNMODE = 'CREDIT') THEN CASE WHEN LEFT(VCHRNO,2) IN (@V3,@V4,@V5) THEN NETAMNT * -1 ELSE NETAMNT END ELSE null END AS CREDIT,
				DIVISION FROM RMD_TRNMAIN A WHERE 
				LEFT(A.VCHRNO,2) IN (@V1,@V4,@V5) AND A.DIVISION LIKE @DIV AND (A.TRNDATE >=@DATE1 AND TRNDATE <= @DATE2) AND ISNULL(A.PARAC,A.TRNAC) LIKE @PARTY
				) AS Z 
