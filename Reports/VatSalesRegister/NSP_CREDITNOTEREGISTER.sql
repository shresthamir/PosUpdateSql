...
BEGIN
	...
	COALESCE(NULLIF(A.BILLTO,''), CP.CUSTNAME, M.FNAME, C.ACNAME,'') PARTICULARS,
	COALESCE(NULLIF(A.BILLTOTEL,''), CP.PANNO, M.PANNO, C.VATNO,'') VATNO,
	...
	FROM SALES_TRNMAIN A JOIN SALES_TRNPROD B ON A.VCHRNO = B.VCHRNO
	LEFT JOIN CustomerProfile CP ON A.RECEIVEBY = CP.CUSTID
	LEFT JOIN MEMBERSHIP M ON M.MEMID = A.MEMBERNO
	LEFT JOIN RMD_ACLIST C ON A.PARAC = C.ACID 
	WHERE A.VoucherType in ('CN','SR') AND B.SNO < 1000
