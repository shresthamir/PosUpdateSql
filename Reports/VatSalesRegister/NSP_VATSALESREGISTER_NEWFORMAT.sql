...
--SELECT @DATE1 = '2023-07-17', @DATE2 = '2024-05-13', @DIV = '%', @V1  = 'SI', @V2 ='CN', @REPMODE = 1
...
IF @REPMODE = 0
	BEGIN
		...
		(
			...
			CASE WHEN TM.VoucherType IN ('SI', 'TI') THEN (TP.TAXABLE + TP.NONTAXABLE) ELSE (TP.TAXABLE + TP.NONTAXABLE)*-1 END SALESAMOUNT,
			...
			*/
			...
			(TP.TAXABLE + TP.NONTAXABLE)*-1 SALESAMOUNT,	(TP.NONTAXABLE)*-1 NONTAXABLE,	NULL EXPORTSALES,
...
ELSE IF @REPMODE = 1
	BEGIN
		...
		(
			...
			SUM((TAXABLE+NONTAXABLE)) SALESAMOUNT,SUM(NONTAXABLE) NONTAXABLE,NULL EXPORTSALES, SUM(DCAMNT) DISCOUNT, SUM(TAXABLE) TAXABLE, SUM(VATAMNT) VAT,
			...
			SUM((TAXABLE+NONTAXABLE))*-1 SALESAMOUNT,SUM(NONTAXABLE)*-1 NONTAXABLE,NULL EXPORTSALES, SUM(DCAMNT)*-1 DISCOUNT,SUM(TAXABLE)*-1 TAXABLE,  SUM(VATAMNT)*-1 VAT,
			...
			FROM 
			(
				SELECT VCHRNO,TRNDATE, BSDATE, TOTAMNT,DCAMNT,TAXABLE+NONTAXABLE+DCAMNT GROSSSALESAMOUNT, TAXABLE+NONTAXABLE SALESAMOUNT, TAXABLE,VATAMNT AS VAT, 
			...
			SUM((TAXABLE+NONTAXABLE+DCAMNT))*-1 GROSSSALESAMOUNT,
			SUM((TAXABLE+NONTAXABLE))*-1 SALESAMOUNT,SUM(NONTAXABLE)*-1 NONTAXABLE, NULL EXPORTSALES, SUM(DCAMNT)*-1 DISCOUNT, SUM(TAXABLE)*-1 TAXABLE, SUM(VATAMNT)*-1 VAT,
			'CN0000' VNO,DIVISION,'CN1' XVNO FROM RMD_TRNMAIN WHERE VoucherType LIKE @V2 AND (TRNDATE > = @DATE1 AND TRNDATE < = @DATE2) AND DIVISION LIKE @DIV and TRNMODE not in ('Supplier','Customer')
			GROUP BY TRNDATE,BSDATE,DIVISION	
		) A
