...
AS
--SELECT @BOOKINGID = 'HB2-MMX-81/82'
...
(
	...
	(
		SELECT CASE WHEN DATEDIFF(D,CD.CheckInDate, GETDATE()) = 0 THEN 1 ELSE DATEDIFF(D,CD.CheckInDate, IIF(CS.[STATUS] ='TRANSFERRED', CD.CheckOutDate, GETDATE())) END interval, CD.* FROM htl_CheckIn_Details CD 
		JOIN HTL_CHECKIN_STATUS CS ON CD.CheckInId = CS.CheckInId WHERE CD.BookingId = @bookingId AND [STATUS] IN ('CHECKED IN', 'TRANSFERRED')
	) A 
) A
ORDER BY STAMP
